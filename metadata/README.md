...
# options: --event-id <id>  --out <base_dir>  --delay <seconds>
...
version control — only the `crawl.py` / `aggregate.py` code is committed.
...
data/                           # git-ignored; accumulates one dir per match
 <slug>/                        # e.g. data/portugal_v_uzbekistan/
  ...
  sources/                      # the lean raw JSON metadata.json is built from
    event.json statistics.json lineups.json incidents.json shots.json
    momentum.json best_players.json comments.json highlights.json
    standings_total.json
    players/<pid>_event_statistics.json
  manifest.jsonl                # one line per request: url, status, size, headers, ts
  summary.json                  # discovered ids + status counts
```

`sources/` is kept so `metadata.json` can be regenerated offline by re-running
...
| `events` | unified, time-ordered timeline (goals, shots, cards, subs, VAR, unmatched highlight clips). Each has `match_seconds` (exact for shots, approx for incidents), a generated NL `text`, and `clip_url` when a highlight matched |
